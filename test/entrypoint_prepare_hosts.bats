...
function setup() {
    ...
    #stub printf    # printf can not be mock
    stub grep
    stub echo
...
@test '#prepare_hosts should return 0 if all instructions has succeeded' {
    ...
    [[ "$(stub_called_times ed)"        -eq 1 ]]
    [[ "$(stub_called_times grep)"      -eq 1 ]]
    [[ "$(stub_called_times echo)"      -eq 0 ]]

    stub_called_with_exactly_times sed 1 '-e' 's/\./\\./g'
    #stub_called_with_exactly_times printf 1 '-i' '-e' 's/192\.168\.1\.72 .*/192.168.1.72 bdc01 bdc01.corp.mysite.example.com/g' '/etc/hosts'
    stub_called_with_exactly_times ed 1 '-s' '/etc/hosts'
